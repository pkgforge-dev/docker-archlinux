...
#---------------------------------------------------------------------------#
# The recorded set matches what the image itself says is installed.
#
# ⛔ Not a comparison against package_count, which gen-evidence derives from
# .packages and so can never disagree with it. This asks pacman inside the image
# and compares that, which is what catches the generator dropping an entry while
# parsing the local database by hand.
#---------------------------------------------------------------------------#
installed="$("$RUNTIME" run --rm --platform "$PLATFORM" "$IMAGE" \
  sh -c 'pacman -Q 2>/dev/null | wc -l' 2>/dev/null | tr -d '[:space:]')"
recorded="$(jq -r '.packages | length' "$EVIDENCE")"
if [ -z "$installed" ]; then
  fail "the image reports how many packages it has" \
    "pacman -Q produced nothing inside $IMAGE on $PLATFORM" \
    "reproduce: $RUNTIME run --rm --platform $PLATFORM $IMAGE sh -c 'pacman -Q | wc -l'"
elif [ "$installed" = "$recorded" ]; then
  ok "the evidence records all $recorded packages the image reports installed"
else
  fail "the evidence records every package the image reports installed" \
    "the image says $installed, the evidence records $recorded" \
    "gen-evidence parses /var/lib/pacman/local by hand, so a parsing fault drops entries silently" \
    "reproduce: $RUNTIME run --rm --platform $PLATFORM $IMAGE sh -c 'pacman -Q | wc -l', then jq '.packages | length' $EVIDENCE"
fi
