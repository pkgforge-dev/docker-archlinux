...
| `80-docs-claims.sh` | every `examples/*.sh` and every fenced `bash` block in the three README files parses, the README links to the repository that exists and names both publish targets, the documented tag names and the ones `scripts/tag-names` emits are the same set, no shipped `pacman.conf` withholds any path, and the README says so |
